...
function EquityCompare({ data }) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(ref.current, { width: ref.current.clientWidth, height: ref.current.clientHeight, layout: { background: { type: ColorType.Solid, color: "#fff" }, textColor: "#131722" }, grid: { vertLines: { color: "#f1f3f6" }, horzLines: { color: "#f1f3f6" } }, rightPriceScale: { borderColor: "#d6dbe3" }, timeScale: { borderColor: "#d6dbe3" } });
...
export function BacktestReport({ url, symbol, market, onClose }) {
  ...
  return <section className="native-report"><header><strong>{report.title}</strong><button className="icon-button" type="button" title="关闭" onClick={onClose}><Icon name="close" /></button></header><div className="native-report-scroll"><div className="report-metrics">{report.metrics.map((item) => <span key={item.label}><small>{item.label}</small><strong>{item.value}</strong></span>)}</div>{report.benchmark ? <EquityCompare data={report.benchmark} /> : null}{report.chart ? <LazyStrategyChart market={market} symbol={symbol} title="K线、均线、成交量与交易点" data={report.chart} className="backtest-native-chart" /> : null}<details className="native-trades"><summary>交易明细 · {report.trades.length} 笔</summary><div className="table-wrap"><table><thead><tr>{report.headers.map((header) => <th key={header}>{header}</th>)}</tr></thead><tbody>{report.trades.map((row, index) => <tr key={index}>{row.map((cell, cellIndex) => <td key={cellIndex}>{cell}</td>)}</tr>)}</tbody></table></div></details></div></section>;
}
