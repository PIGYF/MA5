  ...
  return <>
    <FilterSection title="超跌定义" note="RSI + BIAS + 跌幅">
      <div className="advanced-grid">
        ...
        <Input form={form} setForm={setForm} name="bias_short_length" label="短期BIAS周期" step="1" />
        <Input form={form} setForm={setForm} name="oversold_bias_short" label="短期BIAS阈值（%）" />
        <Input form={form} setForm={setForm} name="bias_mid_length" label="中期BIAS周期" step="1" />
...
function ReboundResult({ result }) {
  ...
  const metrics = [
    ...
    ["超跌事件", result.summary?.setups ?? 0], ["反弹信号", result.summary?.signals ?? 0],
  ];
  return <section className="rebound-result"><div className="report-metrics">{metrics.map(([label, value]) => <span key={label}><small>{label}</small><strong>{value}</strong></span>)}</div><ReboundChart payload={result.chart} title={`${result.symbol} · 超跌反弹`} /><details className="native-trades"><summary>交易明细 · {result.trades?.length || 0} 笔</summary><div className="table-wrap"><table><thead><tr><th>信号日</th><th>买入日</th><th>卖出日</th><th>收益率</th><th>持仓</th><th>退出原因</th></tr></thead><tbody>{result.trades?.length ? result.trades.map((trade, index) => <tr key={`${trade.entry_date}-${index}`}><td>{trade.entry_signal_date}</td><td>{trade.entry_date}</td><td>{trade.exit_date}</td><td className={Number(trade.pnl_pct) >= 0 ? "gain-up" : "gain-down"}>{numberText(trade.pnl_pct)}%</td><td>{trade.bars_held}日</td><td>{trade.reason}</td></tr>) : <tr><td colSpan="6">区间内没有完成交易</td></tr>}</tbody></table></div></details></section>;
}

export function ReboundBacktest({ defaults }) {
  const [form, setForm] = usePersistentState("rebound.backtest.form", () => defaultForm(defaults?.end));
...
export function ReboundWatchlist() {
  ...
  const [form, setForm] = usePersistentState("rebound.watchlist.form", () => defaultForm());
